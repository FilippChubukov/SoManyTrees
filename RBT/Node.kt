...
class Node<K: Comparable<K>, V>(var key: K, var value: V, var parent: Node<K, V>? = null, var Black: Boolean = false) {//Black=true, Red=false
    ...
    fun Lrotate() {
        ...
        if (this == par?.left)  par.left = rightChild

        else if (this == par?.right) par.right = rightChild
    ...
    fun Rrotate() {
        ...
        if (this == par?.left) par.left = leftChild

        else if(this == par?.right) par.right = leftChild
